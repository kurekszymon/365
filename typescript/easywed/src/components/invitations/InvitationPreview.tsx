...
import { BoldIcon, ItalicIcon, MoveIcon, PrinterIcon, UnderlineIcon, XIcon } from "lucide-react"
import {
  DndContext,
  DragOverlay,
  PointerSensor,
...
function InlineTextarea({
  fieldKey, draft, onDraftChange, onCommit, onCancel,
}: {
  fieldKey: keyof InvitationTexts; draft: string
  onDraftChange: (v: string) => void; onCommit: () => void; onCancel: () => void
}) {
  return (
    <textarea
      value={draft}
      ...
      style={{
        position: "absolute", inset: 0, width: "100%", height: "100%", minHeight: "1.5em",
        background: "rgba(255,255,255,0.92)", border: "1.5px solid rgba(59,130,246,0.8)",
        borderRadius: "2px", resize: "none", fontFamily: "inherit", fontSize: "inherit",
        color: "inherit", textAlign: "inherit", padding: "2px 4px", zIndex: 10,
...
function SortableItem({
  id, scale, children, isSelected, isEditing, isPlaceholder, isSeparator,
  onSelect, onEdit, onContext, editOverlay, onRefCapture,
}: {
  id: string; scale: number; children: React.ReactNode
  isSelected: boolean; isEditing: boolean; isPlaceholder: boolean; isSeparator: boolean
  onSelect: (id: string) => void; onEdit: (id: string) => void
  onContext: (id: string, e: React.MouseEvent) => void
  editOverlay: React.ReactNode; onRefCapture?: (el: Element | null) => void
}) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } =
    useSortable({ id, data: { isSeparator, overlay: children } })

  const composedRef = useCallback(
    (el: HTMLDivElement | null) => { setNodeRef(el); onRefCapture?.(el) },
  ...
  return (
    <div
      ...
      style={{
        ...
        outline: isSelected && !isEditing ? "2px solid rgba(59,130,246,0.7)" : undefined,
        outlineOffset: "2px", borderRadius: "2px",
      }}
      {...attributes}
      {...(!isEditing ? listeners : {})}
      onClick={(e) => { e.stopPropagation(); onSelect(id) }}
      onDoubleClick={(e) => { e.stopPropagation(); onEdit(id) }}
      onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); onContext(id, e) }}
      className={isPlaceholder && !isEditing ? "invitation-placeholder" : undefined}
    >
      {children}
      {editOverlay}
    </div>
...
function FreeDraggableField({
  id, pos, scale, children, isSelected, isEditing, isPlaceholder, isSeparator,
  onSelect, onEdit, onContext, editOverlay, onRefCapture,
}: {
  id: string; pos: { x: number; y: number }; scale: number; children: React.ReactNode
  isSelected: boolean; isEditing: boolean; isPlaceholder: boolean; isSeparator: boolean
  onSelect: (id: string) => void; onEdit: (id: string) => void
  onContext: (id: string, e: React.MouseEvent) => void
  editOverlay: React.ReactNode; onRefCapture?: (el: Element | null) => void
  ...
  return (
    <div
      ...
      style={{
        ...
        minWidth: isSeparator ? 160 : undefined,
        transform: transform ? `translate(${transform.x / scale}px, ${transform.y / scale}px)` : undefined,
        cursor: isEditing ? "default" : isDragging ? "grabbing" : "pointer",
        userSelect: "none", touchAction: "none",
        outline: isSelected && !isEditing ? "2px solid rgba(59,130,246,0.7)" : undefined,
        outlineOffset: "2px", borderRadius: "2px",
...
function ToolbarDivider() {
  return <div style={{ width: "1px", height: "16px", background: "rgba(255,255,255,0.2)", margin: "0 2px" }} />
}
...
function ToolbarButton({ active, onClick, title, children }: {
  ...
  return (
    <button type="button" onClick={onClick} title={title}
      className={cn("flex items-center justify-center rounded p-1 transition-colors",
        active ? "bg-white/30 text-white" : "text-white/60 hover:bg-white/15 hover:text-white")}
    >
      {children}
    </button>
  )
...
function FloatingToolbar({
  selectedId, pos, fieldFonts, fieldFormats, separatorStyles, separatorConfigs,
  onSetFieldFont, onSetFieldFormat, onSetSeparatorStyle, onSetSeparatorConfig, onRemoveSeparator, onDeselect,
}: {
  ...
  onSetFieldFormat: (key: keyof InvitationTexts, fmt: Partial<FieldFormat>) => void
  ...
  const fieldKey = !isSep ? (selectedId as keyof InvitationTexts) : null
  const fmt = fieldKey ? (fieldFormats[fieldKey] ?? {}) : null
  const currentFontId = fieldKey ? (fieldFonts[fieldKey] ?? "__default__") : null
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: 30, display: "flex", alignItems: "center", gap: "2px",
        background: "rgba(12,12,12,0.9)", borderRadius: "7px", padding: "4px 6px",
    ...
    >
      {fieldKey && fmt !== null && (
        <>
          <ToolbarButton active={!!fmt.bold} onClick={() => onSetFieldFormat(fieldKey, { bold: fmt.bold ? undefined : true })} title="Bold">
            <BoldIcon className="h-3 w-3" />
          </ToolbarButton>
          <ToolbarButton active={!!fmt.italic} onClick={() => onSetFieldFormat(fieldKey, { italic: fmt.italic ? undefined : true })} title="Italic">
            <ItalicIcon className="h-3 w-3" />
          </ToolbarButton>
          <ToolbarButton active={!!fmt.underline} onClick={() => onSetFieldFormat(fieldKey, { underline: fmt.underline ? undefined : true })} title="Underline">
            <UnderlineIcon className="h-3 w-3" />
          </ToolbarButton>
          <ToolbarDivider />
          {currentFontId !== null && (
            <Select value={currentFontId} onValueChange={(id) => onSetFieldFont(fieldKey, id === "__default__" ? null : id)}>
              <SelectTrigger className="h-6 border-white/20 bg-white/10 text-white hover:bg-white/20" style={{ minWidth: "110px", fontSize: "11px" }}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__default__">{t("invitations.field_font_default")}</SelectItem>
      ...
      {isSep && currentSepStyle !== null && sepCfg !== null && (
        <>
          ...
          {SEPARATOR_STYLE_OPTIONS.map((opt) => (
            <ToolbarButton key={opt.id} active={currentSepStyle === opt.id}
              onClick={() => onSetSeparatorStyle(selectedId, opt.id)} title={opt.id}>
              <span style={{ fontSize: "11px", lineHeight: 1, fontFamily: "serif" }}>{opt.ornament}</span>
            </ToolbarButton>
          ))}
          <ToolbarDivider />
          ...
          >
            <span style={{ fontSize: "9px", color: "rgba(255,255,255,0.4)", userSelect: "none" }}>W</span>
            <input
              type="range" min={20} max={100} step={5}
              value={currentWidthPct}
              onChange={(e) => onSetSeparatorConfig(selectedId, { widthPct: Number(e.target.value) })}
              style={{ width: "54px", accentColor: "rgba(255,255,255,0.8)", cursor: "pointer" }}
            />
            <span style={{ fontSize: "9px", color: "rgba(255,255,255,0.4)", minWidth: "22px", userSelect: "none" }}>
              {currentWidthPct}%
            </span>
          </div>
          <ToolbarDivider />
          ...
          {THICKNESS_OPTIONS.map(({ value, label }) => (
            <ToolbarButton
              ...
              onClick={() => onSetSeparatorConfig(selectedId, { thicknessPx: value })}
              title={`${value}px`}
            >
              <span style={{ fontSize: value <= 1 ? "12px" : "14px", lineHeight: 1, fontWeight: value >= 4 ? 900 : 400 }}>
                {label}
              </span>
            </ToolbarButton>
          ))}
          <ToolbarDivider />
          {/* Remove */}
          <ToolbarButton onClick={() => { onRemoveSeparator(selectedId); onDeselect() }} title={t("invitations.separator_remove")}>
            <XIcon className="h-3 w-3 text-red-400" />
          </ToolbarButton>
        </>
      )}
      <ToolbarDivider />
      <ToolbarButton onClick={onDeselect} title={t("invitations.toolbar_deselect")}>
...
function ContextMenu({
  menu, snapEnabled,
  onAddSeparatorNear, onAddSeparatorAtPos, onMoveToSide, onClose,
}: {
  menu: ContextMenuState
  snapEnabled: boolean
  onAddSeparatorNear: (nearId: string, pos: "before" | "after") => void
  onAddSeparatorAtPos: (side: InvitationSide, pos: { x: number; y: number }) => void
  ...
  return (
    <div
      onMouseDown={(e) => e.stopPropagation()}
      style={{
        position: "absolute", top: clampedY, left: clampedX, zIndex: 30,
        width: MENU_W, background: "rgba(12,12,12,0.95)", borderRadius: "8px",
    ...
    >
      ...
      {/* Snap mode on empty background */}
      {snapEnabled && !targetId && item(
        side === "front" ? t("invitations.context_add_front") : t("invitations.context_add_back"),
...
const PREVIEW_KEY_MAP: Partial<Record<keyof InvitationTexts, string>> = {
  headline: "preview_headline", coupleNames: "preview_couple",
  date: "preview_date", time: "preview_time", venue: "preview_venue",
  venueAddress: "preview_venue_address", rsvpEmail: "preview_rsvp_email",
  rsvpDeadline: "preview_rsvp_deadline",
}
...
export function InvitationPreview() {
  ...
  const [activeOverlayContent, setActiveOverlayContent] = useState<React.ReactNode>(null)
  const [editingField, setEditingField] = useState<keyof InvitationTexts | null>(null)
  const [editingDraft, setEditingDraft] = useState("")
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [toolbarPos, setToolbarPos] = useState<{ top: number; left: number } | null>(null)
  const [contextMenu, setContextMenu] = useState<ContextMenuState | null>(null)
  const [clipboard, setClipboard] = useState<{ key: keyof InvitationTexts; text: string; fontId?: string } | null>(null)
  ...
  useEffect(() => {
    if (!selectedId || !cardRef.current) { setToolbarPos(null); return }
    ...
    const relLeft = Math.max(70, Math.min(PREVIEW_W - 70, (elRect.left + elRect.right) / 2 - cardRect.left))
    // Prefer above; fall back to below if element is near the top
    const top = relTop > TOOLBAR_H + TOOLBAR_GAP
      ? relTop - TOOLBAR_H - TOOLBAR_GAP
      : relBottom + TOOLBAR_GAP
    setToolbarPos({ top, left: relLeft })
  }, [selectedId])

  // previewTexts + placeholder set
  const placeholderFields = new Set<keyof InvitationTexts>()
  const previewTexts: InvitationTexts = { ...design.texts }
  for (const rawKey of Object.keys(design.texts)) {
    const key = rawKey as keyof InvitationTexts
    if (!design.texts[key]) {
      const suffix = PREVIEW_KEY_MAP[key]
      if (suffix) { previewTexts[key] = t(`invitations.gallery.${suffix}`); placeholderFields.add(key) }
    }
  }

  function commitEdit() {
    if (editingField === null) return
    updateTexts({ [editingField]: editingDraft.trim() })
    setEditingField(null); setEditingDraft("")
  }
  function cancelEdit() { setEditingField(null); setEditingDraft("") }
  function deselect() { setSelectedId(null); setToolbarPos(null) }

  function handleSelect(id: string) {
    if (!isSeparatorId(id) && !isFieldKey(id)) return
  ...
  function handleCardContext(e: React.MouseEvent) {
    ...
    const x = e.clientX - cardRect.left
    const y = e.clientY - cardRect.top
    setContextMenu({ x, y, cardX: x / scale, cardY: y / scale, targetId: null, side: previewSide })
  }
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      if (editingField !== null) return
      if (e.key === "Escape") { setContextMenu(null); deselect(); return }
      ...
      if ((e.key === "Delete" || e.key === "Backspace") && selectedId && isSeparatorId(selectedId)
          && document.activeElement === document.body) {
        removeSeparator(selectedId); deselect(); return
      }
    }
    window.addEventListener("keydown", onKeyDown)
    return () => window.removeEventListener("keydown", onKeyDown)
  }, [editingField, selectedId, clipboard, design.texts, design.fieldFonts, undo, redo, updateTexts, setFieldFont, removeSeparator])
  ...
  const restrictToCard: Modifier = ({ transform, draggingNodeRect }) => {
    ...
    return {
      ...transform,
      x: Math.max(card.left - draggingNodeRect.left, Math.min(card.right - draggingNodeRect.right, transform.x)),
      y: Math.max(card.top - draggingNodeRect.top, Math.min(card.bottom - draggingNodeRect.bottom, transform.y)),
    }
  }
  ...
  const sharedProps = {
    colorScheme: design.colorScheme, fontCss,
    fieldFonts: design.fieldFonts, fieldFormats: design.fieldFormats,
    separatorStyles: design.separatorStyles, separatorConfigs: design.separatorConfigs,
    fieldSides: design.fieldSides, fieldOrder: design.fieldOrder,
  }

  const sideFields = design.fieldOrder.filter((k) => design.fieldSides[k] === previewSide)

  function getDefaultFreePos(id: string) {
    const ownSide = design.fieldSides[id]
    const ownSideFields = design.fieldOrder.filter((k) => design.fieldSides[k] === ownSide)
  ...
  function handleSnapDragEnd(event: DragEndEvent) {
    ...
    let sideIdx = 0
    reorderFields(design.fieldOrder.map((k) =>
  ...
  function handleDragStart(event: DragStartEvent) {
    ...
    if (isSeparatorId(id)) {
      setActiveOverlayContent(
        <div style={{ width: `${CARD_W}px`, height: 1, backgroundColor: colorTokens.border, opacity: 0.7 }} />
      )
    } else {
      setActiveOverlayContent(
        (event.active.data.current?.overlay as React.ReactNode | undefined) ?? null
      )
    }
  }

  function handleDragEnd(event: DragEndEvent) {
    if (snapEnabled) { handleSnapDragEnd(event) }
    else {
    ...
    setActiveId(null); setActiveOverlayContent(null)
  }

  function handleDragCancel() { setActiveId(null); setActiveOverlayContent(null) }
  ...
  function wrapFieldSnap(id: string, content: React.ReactNode) {
    return <SortableItem key={id} id={id} scale={scale} {...commonItemProps(id)}>{content}</SortableItem>
  }

  function wrapFieldFree(id: string, content: React.ReactNode) {
    const pos = design.fieldPositions[id] ?? getDefaultFreePos(id)
    return <FreeDraggableField key={id} id={id} pos={pos} scale={scale} {...commonItemProps(id)}>{content}</FreeDraggableField>
  }

  function wrapFieldPrint(id: string, content: React.ReactNode) {
    const pos = design.fieldPositions[id] ?? getDefaultFreePos(id)
    return <div key={id} style={{ position: "absolute", left: pos.x, top: pos.y }}>{content}</div>
  }

  const handlePrintPreview = () => { flushSync(() => setPrintAll(false)); window.print() }
  const handlePrintAll = () => { flushSync(() => setPrintAll(true)); window.print() }
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          {printAll && guests && guests.length > 0 ? (
            guests.map((name, idx) => (
              <Fragment key={`${idx}-${name}`}>
                <Component texts={design.texts} guestName={name} side="front" wrapField={!snapEnabled ? wrapFieldPrint : undefined} {...sharedProps} />
                <Component texts={design.texts} guestName={name} side="back" wrapField={!snapEnabled ? wrapFieldPrint : undefined} {...sharedProps} />
              </Fragment>
            ))
          ) : (
            <>
              <Component texts={design.texts} side="front" wrapField={!snapEnabled ? wrapFieldPrint : undefined} {...sharedProps} />
              <Component texts={design.texts} side="back" wrapField={!snapEnabled ? wrapFieldPrint : undefined} {...sharedProps} />
            </>
          )}
        </div>,
        document.body
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={handleDragCancel} modifiers={snapEnabled ? undefined : [restrictToCard]}>
        {/*
        ...
        */}
        <div style={{ position: "relative", width: PREVIEW_W, height: scaledH, flexShrink: 0 }}>
          <div
            ref={cardRef}
            className="overflow-hidden rounded-lg border shadow-sm"
            style={{ width: PREVIEW_W, height: scaledH }}
            onClick={() => { deselect(); cancelEdit(); setContextMenu(null) }}
            onContextMenu={handleCardContext}
          >
            {snapEnabled ? (
              <SortableContext items={sideFields} strategy={verticalListSortingStrategy}>
                <div style={scaledCardStyle}>
                  <Component texts={previewTexts} side={previewSide} wrapField={wrapFieldSnap} {...sharedProps} />
                </div>
              </SortableContext>
            ) : (
              <div style={scaledCardStyle}>
                <Component texts={previewTexts} side={previewSide} wrapField={wrapFieldFree} {...sharedProps} />
              </div>
            )}
            ...
            <div className="absolute top-2 left-2 flex overflow-hidden rounded-md border border-white/20 shadow-md">
              {(["front", "back"] as const).map((s) => (
                <button key={s} type="button"
                  onClick={(e) => { e.stopPropagation(); setPreviewSide(s) }}
                  className={cn("px-3 py-1 text-xs font-semibold transition-colors",
                    previewSide === s ? "bg-black/60 text-white" : "bg-black/20 text-white/70 hover:bg-black/40 hover:text-white")}
                >
                  {s === "front" ? t("invitations.side_front") : t("invitations.side_back")}
                </button>
              ))}
            </div>
            ...
            <button type="button"
              ...
              title={snapEnabled ? t("invitations.snap_on") : t("invitations.snap_off")}
              className={cn("absolute top-2 right-2 flex items-center gap-1 rounded-md border border-white/20 px-2 py-1 text-xs font-semibold shadow-md transition-colors",
                snapEnabled ? "bg-black/60 text-white" : "bg-black/20 text-white/70 hover:bg-black/40 hover:text-white")}
            >
          ...
          <FloatingToolbar
            selectedId={selectedId} pos={toolbarPos}
            fieldFonts={design.fieldFonts} fieldFormats={design.fieldFormats}
            separatorStyles={design.separatorStyles} separatorConfigs={design.separatorConfigs}
            onSetFieldFont={setFieldFont} onSetFieldFormat={setFieldFormat}
            onSetSeparatorStyle={setSeparatorStyle} onSetSeparatorConfig={setSeparatorConfig}
            onRemoveSeparator={removeSeparator} onDeselect={deselect}
          />
          {contextMenu && (
            <ContextMenu
              menu={contextMenu} snapEnabled={snapEnabled}
        ...
        <DragOverlay dropAnimation={null}>
          {snapEnabled && activeOverlayContent ? (
            ...
            <div style={{
              ...
              alignItems: design.template === "modern" ? "flex-start" : "center",
              fontFamily: fontCss, color: colorTokens.text,
              userSelect: "none", touchAction: "none",
            }}>
