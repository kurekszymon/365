...
describe("toBlockingWeddings", () => {
  ...
  it("blocks when the owner isn't in their own member list", () => {
    // An owner deleting their own membership row would leave a wedding whose
    // owner isn't in its own member list. RLS is meant to prevent that; if it
    // ever regresses, the co-member is still real and must still block.
    const rows = [
