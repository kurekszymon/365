...
describe("formatMoney", () => {
  it("formats minor units as a Polish price", () => {
    // Intl's exact spacing is an ICU detail, so the assertion is on the parts
    // that are ours: the amount, the comma decimal separator, and the symbol.
  ...
  // Why this is a function and not an inline `Intl.NumberFormat`: it is handed
  // `i18n.language`, validated nowhere, and Intl raises `RangeError` on a tag it
  // cannot parse - an uncaught throw in a price cell takes down the CRM screen.
  ...
  // The near miss, pinned so nobody re-derives it: `^[A-Z]{3}$` is exactly
  // Intl's well-formedness rule, so a stored `ZZZ` formats with the code
  // standing in for the symbol rather than throwing.
...
describe("parsePriceInput", () => {
  ...
  // The round trip: `formatMoney` in Polish emits no-break spaces for grouping
  // and before the symbol, so a pasted price only parses because of the strip.
  ...
  // `CrmMenuPackageEditor` seeds the price field with `formatMoney(…,
  // i18n.language)`, which on an English UI is `PLN 1,405.00` - so mapping `,`
  // to `.` unconditionally makes the field unreadable to its own output.
  ...
  // The same round trip for a currency whose minor-unit exponent is not 2. JPY
  // and KWD are reachable (shape-checked, not allowlisted), and Intl's defaults
  // would print `¥1,405` and `KWD 405.000` - both of which this parser refuses
  // as ambiguous. `formatMoney` pins two digits so its output stays readable.
  ...
  // grouping after the decimal mark, so the rightmost one is it.
  ...
  // it. `1 000 000` is the ceiling, so this is also the largest amount the field
  // will ever read back.
  ...
  // English venue and 1.405 to a Polish one, and nothing breaks the tie.
  ...
  // Why this is a hand-written parser: `Math.round(4.0559 * 100)` rounds up, and
  // `4.055 * 100` is `405.49999999999994` so it rounds *down* past the digit it
  // should have kept. Nothing here multiplies a fraction, so extra digits are
  // dropped - a prefix of what was typed, not an amount nobody wrote.
  //
  // The examples avoid a bare three-digit fraction: that shape is refused above
  // as ambiguous grouping.
